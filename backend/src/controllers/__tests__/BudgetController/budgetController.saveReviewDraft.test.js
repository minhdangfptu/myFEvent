...
describe('budgetController.saveReviewDraft', () => {
  it('[Abnormal] TC01 - should return 404 when budget not found', async () => {
    ...
    await budgetController.saveReviewDraft(req, res);

    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith({ message: 'Budget not found' });
  });

  it('[Normal] TC02 - should update review feedback and status for items', async () => {
    const { ensureEventExists, ensureDepartmentInEvent } = await import('../../../services/departmentService.js');
    const { _mockFindOne } = await import('../../../models/budgetPlanDep.js');
    ensureEventExists.mockResolvedValue(true);
    ensureDepartmentInEvent.mockResolvedValue({ _id: 'd1' });
    ...
    const budget = {
      items: [
        { itemId: 'i1', name: 'Old 1', status: 'pending', feedback: '' },
        { itemId: 'i2', name: 'Old 2', status: 'pending', feedback: '' },
      ],
      save,
    };
    ...
    const req = {
      ...
      body: {
        items: [
          { itemId: 'i1', status: 'approved', feedback: 'ok' },
        ],
      },
    };
    const res = mockRes();
